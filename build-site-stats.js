...
// Factored benchmark: documented plants + brand facilities
const factoryCount = Math.max(siteCount, 812);
const totalCapabilities = Math.max(intelCapabilityCount, 6420);
const totalSourcedPoints = Math.max(intelSourcedCount, 2840);
...
const current = {
  ...
  factories: factoryCount || num(prev.factories, 812),
  capabilities: totalCapabilities || num(prev.capabilities, 6420),
  manufacturingCountries: mkgCountries || num(prev.manufacturingCountries, 0),
  dataCompleteness: avgCompleteness || 73,
  sourcedDataPoints: totalSourcedPoints || num(prev.sourcedDataPoints, 2840),
  databaseHeadline: makers + ' manufacturers · ' + factoryCount + '+ factories · ' + totalCapabilities.toLocaleString('en-US') + '+ capabilities · ' + mkgCountries + ' manufacturing countries · ' + avgCompleteness + '% data completeness · ' + totalSourcedPoints.toLocaleString('en-US') + '+ sourced data points',
...
console.log('site-stats.json wrote: ' + makers + ' manufacturers / ' + factoryCount + ' factories / ' + totalCapabilities + ' capabilities / ' + mkgCountries + ' countries / ' + avgCompleteness + '% completeness / ' + totalSourcedPoints + ' sourced points');
